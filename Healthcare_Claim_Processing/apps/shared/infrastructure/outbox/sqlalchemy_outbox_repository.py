...
class OutboxEventModel(Base):
    ...
    The outbox record is persisted in the same database
    transaction as the Appointment aggregate state change.
    """
...
class SQLAlchemyOutboxRepository(OutboxRepository):
    ...
    The supplied AsyncSession must belong to the transaction
    managed by the Appointment UnitOfWork.
    """
    ...
    async def add(
        ...
        """
        Persist one domain event into the transactional outbox.
    ...
    def _serialize_event(
        ...
        return SQLAlchemyOutboxRepository._make_json_safe(data)
    ...
    def _make_json_safe(
        ...
        """
        Recursively convert values that PostgreSQL JSONB
